...
export function NavProvider({ children }: { children: ReactNode }) {
  ...
  const navigate = useCallback(
    (r: RouteId, p: Record<string, string> = {}) => {
      setRoute(r);
      setParams(p);
  ...
  useEffect(() => {
    ...
    function onMove(e: TouchEvent) {
      ...
      if (dx > 60 && Math.abs(dy) < 40 && startX < 40) {
  ...
  const value = useMemo<NavContextValue>(
    ...
    [route, params, navigate, back, forward, historyIndex, history.length, refreshKey, refresh, sideMenuOpen]
  );
...
export function useAutoLogout(onLogout: () => void) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setWarningSeconds((s) => (s ?? 1) - 1), 1000);
    return () => clearTimeout(t);
  }, [warningSeconds, onLogout]);
